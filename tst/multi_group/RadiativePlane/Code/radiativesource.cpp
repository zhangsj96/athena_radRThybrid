...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
   ...
   for(int i=0; i<prad->nfreq; ++i){
    fscanf(fini,"%lf",&(prad->opacity_spectrum(i)));
   }
  ...
  if(RADIATION_ENABLED || IM_RADIATION_ENABLED){
    ...
    for (int ifr=0; ifr < nfreq-1; ++ifr){
      ...
      for(int i=0; i<ncells1; ++i){
        ...
        prad->sigma_a(k,j,i,ifr) = prad->opacity_spectrum(ifr);      
        prad->sigma_ae(k,j,i,ifr) = prad->opacity_spectrum(ifr);
    ...
    for(int i=0; i<ncells1; ++i){
      ...
      prad->sigma_a(k,j,i,nfreq-1) = prad->opacity_spectrum(nfreq-1);      
