...
BlockFFTGravity::BlockFFTGravity(MeshBlock *pmb, ParameterInput *pin)
  ...
  gtlist_ = new GravityBoundaryTaskList(pin, pmy_mesh_);
  ...
  if ((pmy_mesh_->shear_periodic)&&(gbflag==GravityBoundaryFlag::open)) {
...
#ifdef FFT
#ifdef MPI_PARALLEL
  if ((gbflag==GravityBoundaryFlag::periodic)||(gbflag==GravityBoundaryFlag::open)) {
      ...
      for (int i=0; i<slow_nx1; i++) {
        for (int k=0; k<slow_nx3; k++) {
          ...
          if (((slow_ilo+i) + (slow_jlo+j) + (slow_klo+k)) == 0) {
            kernel = 0.0;
          } else if (pmy_mesh_->shear_periodic) {
      ...
      for (int i=0; i<2*nx1; i++) {
        ...
        for (int pz : {-1, 1}) {
          for (int py : {-1, 1}) {
            for (int px : {-1, 1}) {
              int sgn = pz*py*px;
              Real x = (-gi + px*0.5)*dx1_;
              Real y = (-gj + py*0.5)*dx2_;
              Real z = (-gk + pz*0.5)*dx3_;
              grf_[idx] += sgn*_GetIGF(x, y, z);
            }
          }
        }
        grf_[idx] *= -gconst;
