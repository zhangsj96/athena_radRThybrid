...
Real t_star, r_star, tau0, t_bkg, x1min, cavity_fac, r_c, dgratio, small_grain_ratio, width_tanh, sigma_shadow, amp_shadow, t_growth, prat;

Real c_speed_cgs = 2.99792458e10;
Real G_cgs = 6.6743e-8; //
Real Msun_cgs = 1.989e33; //
Real mean_molecular_weight = 2.3; //
Real N_A = 6.023e23; //
int len_freq;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if(RADIATION_ENABLED || IM_RADIATION_ENABLED){
    ...
    if(RADIATION_ENABLED || IM_RADIATION_ENABLED){
      ...
      Real timeunit = 1./std::sqrt(gm0*G_cgs*Msun_cgs/(lunit*lunit*lunit));
      Real speed_of_light = c_speed_cgs / std::sqrt(gm0*G_cgs*Msun_cgs/lunit);
      Real sigma_b = 0.25*speed_of_light*prat;
      Real k_b = mean_molecular_weight / N_A;
      Real h_planck = 2.*std::pow(PI, 5)*std::pow(k_b,4)/(15.*speed_of_light*speed_of_light*sigma_b);
      h_planck = std::pow(h_planck, 1./3.);
      ...
      for(int ifrs=0; ifrs<len_freq; ifrs++){
        ...
        * dgratio * small_grain_ratio  * rhounit * lunit;
        stellar_opacity_freq(ifrs) = stellar_opacity_freq(ifrs) * timeunit;
        std::cout << h_planck << std::endl;
        std::cout << speed_of_light << std::endl;
        std::cout << stellar_opacity_freq(ifrs) << std::endl;
        std::cout << k_b << std::endl;
        planck_function(ifrs) = 2*h_planck/speed_of_light/speed_of_light
        *stellar_opacity_freq(ifrs)*stellar_opacity_freq(ifrs)*stellar_opacity_freq(ifrs)
        /(std::exp(h_planck*stellar_opacity_freq(ifrs)/(k_b*t_star)) - 1.) * PI;
        std::cout << planck_function(ifrs) << std::endl;
      }
      for(int ifrs=0; ifrs<len_freq; ifrs++){
        Real dfreq;
        if (ifrs==0){
          dfreq = stellar_opacity_freq(1) - stellar_opacity_freq(0);
        }else{
          dfreq = stellar_opacity_freq(ifrs) - stellar_opacity_freq(ifrs-1);
        }
        planck_function(ifrs) = planck_function(ifrs) * dfreq;
      }
      ...
      for (int k=ks; k<=ke; k++) {
        for (int j=js; j<=je; j++) {
          ...
          if (lx1 == 0){
            ...
            // is or is-1?
            pmy_mesh->ruser_mesh_data[0](tk,tj,0) = kappa_star*column_rho_bc(tj);
          }
             ...
             AthenaArray<Real> &cons_scalar)
{
  ...
  for (int k = ks; k <= ke; ++k) {
    ...
    for (int j = js; j <= je; ++j) {
      ...
      //
      tau(0) = pmb->pmy_mesh->ruser_mesh_data[0](tk,tj,lx1);
      for (int i = is; i<= ie; ++i){
        // step 1.
