...
void RadIntegrator::FirstOrderFluxDivergenceSafe(const Real wght, 
                                       AthenaArray<Real> &ir)
{
  Radiation *prad=pmy_rad;
  MeshBlock *pmb=prad->pmy_block;
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for(int i=is; i<=ie; ++i){
        ...
        for(int n=0; n<prad->n_fre_ang; ++n){
          ...
          Real lflux = 0.5 * vl * irln[n] + 0.5 * pvel_ln[n] * irn[n]
                     + 0.5 * smax * irln[n];
          coef1n[n] = -areal * 0.5 * (nvel_ln[n] - smax);
          // right hand side
          smax = s1_rn[n];
          smin = s2_rn[n];
          Real vr = pvel_rn[n] + nvel_rn[n];
          Real rflux = 0.5 * nvel_rn[n] * irn[n] + 0.5 * vr * irrn[n] 
                     - 0.5 * smax * irrn[n];
          coef1n[n] += arear * 0.5 * (pvel_rn[n] + smax);
      ...
      for(int j=js; j<=je; ++j){
        ...
        for(int i=is; i<=ie; ++i){
          ...
          for(int n=0; n<prad->n_fre_ang; ++n){
            ...
            Real lflux = 0.5 * vl * irln[n] + 0.5 * pvel_ln[n] * irn[n]
                       + 0.5 * smax * irln[n];
            coef2n[n] = -areal * 0.5 * (nvel_ln[n] - smax);
          // right hand side
            smax = s1_rn[n];
            smin = s2_rn[n];
            Real vr = pvel_rn[n] + nvel_rn[n];
            Real rflux = 0.5 * nvel_rn[n] * irn[n] + 0.5 * vr * irrn[n] 
                      - 0.5 * smax * irrn[n];
            coef2n[n] += arear * 0.5 * (pvel_rn[n] + smax);
      ...
      for(int j=js; j<=je; ++j){
        ...
        for(int i=is; i<=ie; ++i){
          ...
          for(int n=0; n<prad->n_fre_ang; ++n){
            ...
            Real lflux = 0.5 * vl * irln[n] + 0.5 * pvel_ln[n] * irn[n]
                       + 0.5 * smax * irln[n];
            coef3n[n] = -areal * 0.5 * (nvel_ln[n] - smax);
          // right hand side
            smax = s1_rn[n];
            smin = s2_rn[n];
            Real vr = pvel_rn[n] + nvel_rn[n];
            Real rflux = 0.5 * nvel_rn[n] * irn[n] + 0.5 * vr * irrn[n] 
                     - 0.5 * smax * irrn[n];
            coef3n[n] += arear * 0.5 * (pvel_rn[n] + smax);
