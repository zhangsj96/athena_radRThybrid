...
void RadIntegrator::ImplicitAngularFluxesNonSplit(const Real wght, 
                                     AthenaArray<Real> &ir_ini)
{
  ...
  AthenaArray<Real> &ir_zeta_r = ir_zeta_r_;
    ...
    for(int j=js; j<=je; ++j)
      for(int i=is; i<=ie; ++i){
        ...
        ///////////////////////////////////////////////////////////////////////////////
          ...
          // n is for zeta
          ImplicitPsiFlux(k,j,i,2*nzeta-1,wght,0.0,zeta_coef0+zeta_coef1,ir_ini,imp_ang_coef_);
          // now go from 2*nzeta-2 to 0
          for(int n=2*nzeta-2; n>=0; --n){
            Real g_zeta_coef_l = g_zeta_(n);
            Real g_zeta_coef_r = g_zeta_(n+1);
            Real zeta_coef0 =  wght * g_zeta_coef_l * prad->reduced_c * area_zeta(n);
            Real zeta_coef1 = -wght * g_zeta_coef_r * prad->reduced_c * area_zeta(n+1);
            for(int m=0; m<2*npsi; ++m)
              ir_zeta_r(m) = ir_ini(k,j,i,(n+1)*2*npsi+m) + ang_flx_(k,j,i,(n+1)*2*npsi+m);
            ImplicitPsiFlux(k,j,i,n,wght,zeta_coef1,zeta_coef0,ir_ini,ir_zeta_r);
...
void RadIntegrator::ImplicitPsiFluxNonSplit(int k, int j, int i, int n_zeta, Real wght, Real zeta_coef1, 
            Real zeta_coef, AthenaArray<Real> &ir_ini, AthenaArray<Real> &imp_ang_coef)
{

  Radiation *prad=pmy_rad;
  Coordinates *pco=prad->pmy_block->pcoord;
  AthenaArray<Real> &area_psi = psi_area_, &ang_vol = ang_vol_;
  int &npsi = prad->npsi;
...
//    int ang_psi_r = n_zeta*2+npsi+1;
    ...
    ang_zeta_r = (n_zeta+1)*2*npsi+2*npsi-1;

    g_psi_coef_l = g_psi_(2*npsi-1);
    coef0 = wght * g_psi_coef_l * prad->reduced_c * 
            area_psi(2*npsi-1)/ang_vol(ang_num);
    z_coef1 = zeta_coef1/ang_vol(ang_num);
    z_coef = zeta_coef/ang_vol(ang_num);
    ang_flx_(k,j,i,ang_num) = -z_coef1 * ir_ini(k,j,i,ang_zeta_r);
    imp_ang_coef(k,j,i,ang_num) = coef0 + z_coef;
    ...
    ang_zeta_r=(n_zeta+1)*2*npsi+npsi;
    ...
    for(int j=js; j<=je; ++j)
      for(int i=is; i<=ie; ++i){
        ...
        ///////////////////////////////////////////////////////////////////////////////
          ...
          for(int m=0; m<2*npsi; ++m)
            ir_zeta_r(m) = 0.0;
          ImplicitPsiFlux(k,j,i,2*nzeta-1,wght,0.0,zeta_coef0+zeta_coef1,ir_ini,ir_zeta_r);
          // now go from 2*nzeta-2 to 0
          for(int n=2*nzeta-2; n>=0; --n){
            Real g_zeta_coef_l = g_zeta_(n);
            Real g_zeta_coef_r = g_zeta_(n+1);
...
void RadIntegrator::ImplicitPsiFlux(int k, int j, int i, int n_zeta, Real wght, Real zeta_coef1, 
            Real zeta_coef, AthenaArray<Real> &ir_ini, AthenaArray<Real> &ir_zeta_r)
{
  ...
  // starting from m=0, 
    ...
    Real z_coef1 = zeta_coef1/ang_vol(ang_num);
    Real z_coef = zeta_coef/ang_vol(ang_num);
    Real irnew = (ir_ini(k,j,i,ang_num) - z_coef1 * ir_zeta_r(0))/
                  (1.0 + coef1 + z_coef);
    ang_flx_(k,j,i,ang_num) = irnew - ir_ini(k,j,i,ang_num);
    for(int m=1; m<npsi; ++m){ // all take the left state
      ang_num = n_zeta*2*npsi+m;
      g_psi_coef_l = g_psi_(m);
      g_psi_coef_r = g_psi_(m+1);
      ...
      z_coef1 = zeta_coef1/ang_vol(ang_num);
      z_coef = zeta_coef/ang_vol(ang_num);
      irnew = (ir_ini(k,j,i,ang_num) - coef0 * irnew - z_coef1 * ir_zeta_r(m))/
                   (1.0 + coef1 + z_coef);
      ang_flx_(k,j,i,ang_num) = irnew - ir_ini(k,j,i,ang_num);
    }
            // from npsi to 2*npsi, we take the right state`.
    ...
    z_coef1 = zeta_coef1/ang_vol(ang_num);
    z_coef = zeta_coef/ang_vol(ang_num);
    irnew = (ir_ini(k,j,i,ang_num) - z_coef1 * ir_zeta_r(2*npsi-1))/
            (1.0 + coef0 + z_coef);
    ang_flx_(k,j,i,ang_num) = irnew - ir_ini(k,j,i,ang_num);
    for(int m=2*npsi-2; m>=npsi; --m){
      ang_num = n_zeta*2*npsi+m;
      ...
      z_coef1 = zeta_coef1/ang_vol(ang_num);
      z_coef = zeta_coef/ang_vol(ang_num);
      irnew = (ir_ini(k,j,i,ang_num) - coef1 * irnew - z_coef1 * ir_zeta_r(m))/
                  (1.0 + coef0 + z_coef);
      ang_flx_(k,j,i,ang_num) = irnew - ir_ini(k,j,i,ang_num);              
    }
  }else{// end g_psi(1) < 0
    ...
    Real z_coef1 = zeta_coef1/ang_vol(ang_num);
    Real z_coef = zeta_coef/ang_vol(ang_num);
    Real irnew = (ir_ini(k,j,i,ang_num) - z_coef1 * ir_zeta_r(npsi-1))/
                      (1.0 + coef0 + z_coef);
    ang_flx_(k,j,i,ang_num) = irnew - ir_ini(k,j,i,ang_num);
    for(int m=npsi-2; m>=0; --m){ // all take the right state
      ang_num = n_zeta*2*npsi+m;
    ...
    z_coef1 = zeta_coef1/ang_vol(ang_num);
    z_coef = zeta_coef/ang_vol(ang_num);
    irnew = (ir_ini(k,j,i,ang_num) - z_coef1 * ir_zeta_r(npsi+1))/
                   (1.0 + coef1 + z_coef);
    ang_flx_(k,j,i,ang_num) = irnew - ir_ini(k,j,i,ang_num);
    for(int m=npsi+1; m<2*npsi; ++m){
      ang_num = n_zeta*2*npsi+m;
